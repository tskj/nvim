...
vim.keymap.set({"n", "t"}, "<C-S-H>", _9_, {desc = "Previous Buffer (:bp)"})
local function _10_()
  return vim.api.nvim_command(":bn")
end
vim.keymap.set({"n", "t"}, "<C-S-L>", _10_, {desc = "Next Buffer (:bn)"})
local function _11_()
  return vim.api.nvim_command(":tabnext")
end
vim.keymap.set({"n", "t"}, "<C-S-K>", _11_, {desc = "Next Tab (:tabnext)"})
local function _12_()
  return vim.api.nvim_command(":tabprev")
end
vim.keymap.set({"n", "t"}, "<C-S-J>", _12_, {desc = "Previous Tab (:tabprev)"})
...
vim.keymap.set({"n", "v"}, "<leader>ap", _13_, {desc = "[A]pplication [P]owershell"})
local function _14_()
  return vim.cmd.terminal("wsl.exe zsh")
end
vim.keymap.set({"n", "v"}, "<leader>al", _14_, {desc = "[A]pplication [L]inux (wsl zsh)"})
local function _15_()
  return vim.cmd.terminal("wsl.exe bash")
end
vim.keymap.set({"n", "v"}, "<leader>ab", _15_, {desc = "[A]pplication [B]ash (wsl bash)"})
...
vim.keymap.set({"n", "v"}, "<leader>bh", _16_, {desc = "[B]uffer [H]ome [Mini.Starter]"})
vim.keymap.set({"n", "v"}, "<leader>bD", ":bd!<cr>", {desc = "[B]uffer [D]elete force (:bd!)"})
local function _17_()
  vim.cmd.tabnew()
  return run(require("mini.starter").open)
end
vim.keymap.set({"n", "v"}, "<leader>lh", _17_, {desc = "[L]ayer [H]ome (open new tab with Starter page)"})
...
vim.keymap.set({"n", "v"}, "<leader>lo", _18_, {desc = "[L]ayer [O]nly (open buffer in new tab to 'maximize' it)"})
local function _19_()
  return vim.cmd.tabmove("-1")
end
vim.keymap.set({"n", "v"}, "<leader>l<", _19_, {desc = "[L]ayer [<]eft (move tab left)"})
local function _20_()
  return vim.cmd.tabmove("+1")
end
vim.keymap.set({"n", "v"}, "<leader>l>", _20_, {desc = "[L]ayer [>]ight (move tab right)"})
local function _21_()
  vim.cmd(("cd " .. nvim_dir))
  ...
  return vim.api.nvim_create_autocmd("TermClose", {pattern = "term://*make", callback = _22_})
end
vim.keymap.set({"n", "v"}, "<leader>qr", _21_, {desc = "[Q]uit [R]eload (compiles fennel and quits)"})
...
vim.keymap.set({"n", "v"}, "<leader>s/", _23_, {desc = "[S]earch [/] in Open Files"})
local function _24_()
  return builtin.find_files({cwd = vim.fn.stdpath("config")})
end
vim.keymap.set({"n", "v"}, "<leader>sn", _24_, {desc = "[S]earch [N]eovim config files"})
...
vim.keymap.set({"n", "v"}, "<leader>zn", _25_)
...
vim.keymap.set("n", "[t", _27_, {desc = "[[]ump [T]odo previous"})
local function _28_()
  return run(require("todo-comments").jump_next)
end
return vim.keymap.set("n", "]t", _28_, {desc = "[]]ump [T]odo next"})
