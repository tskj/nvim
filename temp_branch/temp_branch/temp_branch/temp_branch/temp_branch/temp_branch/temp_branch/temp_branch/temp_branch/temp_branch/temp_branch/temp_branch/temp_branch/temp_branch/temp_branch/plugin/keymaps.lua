...
vim.keymap.set({"n", "v"}, "<leader>ap", _14_, {desc = "[A]pplication [P]owershell"})
local function _15_()
  return vim.cmd.terminal("wsl.exe zsh")
end
vim.keymap.set({"n", "v"}, "<leader>al", _15_, {desc = "[A]pplication [L]inux (wsl zsh)"})
local function _16_()
  return vim.cmd.terminal("wsl.exe bash")
end
vim.keymap.set({"n", "v"}, "<leader>ab", _16_, {desc = "[A]pplication [B]ash (wsl bash)"})
...
vim.keymap.set({"n", "v"}, "<leader>bh", _17_, {desc = "[B]uffer [H]ome [Mini.Starter]"})
vim.keymap.set({"n", "v"}, "<leader>bD", ":bd!<cr>", {desc = "[B]uffer [D]elete force (:bd!)"})
local function _18_()
  vim.cmd.tabnew()
  return run(require("mini.starter").open)
end
vim.keymap.set({"n", "v"}, "<leader>lh", _18_, {desc = "[L]ayer [H]ome (open new tab with Starter page)"})
...
vim.keymap.set({"n", "v"}, "<leader>lo", _19_, {desc = "[L]ayer [O]nly (open buffer in new tab to 'maximize' it)"})
local function _20_()
  return vim.cmd.tabmove("-1")
end
vim.keymap.set({"n", "v"}, "<leader>l<", _20_, {desc = "[L]ayer [<]eft (move tab left)"})
local function _21_()
  return vim.cmd.tabmove("+1")
end
vim.keymap.set({"n", "v"}, "<leader>l>", _21_, {desc = "[L]ayer [>]ight (move tab right)"})
local function _22_()
  vim.cmd(("cd " .. nvim_dir))
