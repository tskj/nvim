...
-- Mapping calculate_text_object for use within select_list_item
local calculate_text_object = ____exports.calculate_text_object

local function select_list_item(inside)
  -- Get the current line and cursor position
  local line = vim.fn.getline(".")
  local cursor_index = vim.fn.col(".") - 1  -- Adjust cursor index for Lua (0-based)

  -- Calculate the list item boundaries using calculate_text_object
  local boundaries = calculate_text_object(nil, line, cursor_index)
  if not boundaries then
    print("No list item detected")
    return
  end
  local start_index, end_index = boundaries[1], boundaries[2]

  print("start", start_index, "end", end_index)

  -- Adjust based on inside or around selection
  if inside then
    -- Inside selection: start after any whitespace at the beginning
    vim.fn.setpos(".", {0, vim.fn.line("."), start_index + 1, 0})
    vim.cmd("normal! v")
    vim.fn.setpos(".", {0, vim.fn.line("."), end_index, 0})
  else
    -- Around selection: select the entire list item
...
-- Key mappings for visual and operator-pending modes
vim.keymap.set("x", "al", function() select_list_item(false) end, { desc = "Select around list items" })
vim.keymap.set("x", "il", function() select_list_item(true) end, { desc = "Select inside list items" })

vim.keymap.set("o", "al", function()
  select_list_item(false)
  vim.cmd("normal! gv")
end, { desc = "Operate around list items" })

vim.keymap.set("o", "il", function()
  select_list_item(true)
  vim.cmd("normal! gv")
end, { desc = "Operate inside list items" })
