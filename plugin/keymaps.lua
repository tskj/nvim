...
local function _84_()
  ...
  if string.find(current_fo, "l") then
    ...
    return print("Removed 'l' flag - will reformat existing long lines")
  else
    vim.opt_local.formatoptions:append("l")
    return print("Added 'l' flag - won't reformat existing long lines")
  end
end
return vim.keymap.set({"n", "v"}, "<leader>tw", _84_, {desc = "[T]oggle [W]rap behavior (formatoptions 'l' flag)"})
