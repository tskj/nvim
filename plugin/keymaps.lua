...
vim.keymap.set({"n", "v"}, "<leader>ap", _17_, {desc = "[A]pplication [P]owershell"})
local function _18_()
  return vim.cmd.terminal("wsl.exe zsh")
end
vim.keymap.set({"n", "v"}, "<leader>al", _18_, {desc = "[A]pplication [L]inux (wsl zsh)"})
local function _19_()
  return vim.cmd.terminal("wsl.exe bash")
end
vim.keymap.set({"n", "v"}, "<leader>ab", _19_, {desc = "[A]pplication [B]ash (wsl bash)"})
...
local function register(type)
  ...
  return vim.defer_fn(_33_, 100000)
end
local function _34_()
  if (m_type == "q") then
...
vim.keymap.set("n", ",", _36_)
local function _38_()
  register("q")
  return vim.api.nvim_command("cprev")
end
vim.keymap.set("n", "[q", _38_, {desc = "[[]ump [Q]uickfix previous (:cprev)"})
local function _39_()
  register("q")
  return vim.api.nvim_command("cnext")
end
vim.keymap.set("n", "]q", _39_, {desc = "[]]ump [Q]uickfix next (:cnext)"})
local function _40_()
  register("l")
  return vim.api.nvim_command("lprev")
end
vim.keymap.set("n", "[l", _40_, {desc = "[[]ump [L]ocation previous (:lprev)"})
local function _41_()
  register("l")
  return vim.api.nvim_command("lnext")
end
vim.keymap.set("n", "]l", _41_, {desc = "[]]ump [L]ocation next (:lnext)"})
local function _42_()
  register("d")
  return vim.diagnostic.goto_prev()
end
vim.keymap.set("n", "[d", _42_, {desc = "[[]ump [D]iagnostic previous"})
local function _43_()
  register("d")
  return vim.diagnostic.goto_next()
end
vim.keymap.set("n", "]d", _43_, {desc = "[]]ump [D]iagnostic next"})
local function _44_()
  register("t")
  return run(require("todo-comments").jump_prev)
end
vim.keymap.set("n", "[t", _44_, {desc = "[[]ump [T]odo previous"})
local function _45_()
  register("t")
  return run(require("todo-comments").jump_next)
end
vim.keymap.set("n", "]t", _45_, {desc = "[]]ump [T]odo next"})
...
vim.keymap.set({"n", "x"}, "<leader>rs", _46_, {desc = "[R]efactor [S]earch (telescope)"})
local function _47_()
  return require("refactoring").debug.printf()
end
vim.keymap.set("n", "<leader>rdf", _47_, {desc = "[R]efactor [D]debug [F]unction"})
local function _48_()
  return require("refactoring").debug.print_var()
end
vim.keymap.set({"n", "x"}, "<leader>rdp", _48_, {desc = "[R]efactor [D]debug [P]rint (variable or selection)"})
local function _49_()
